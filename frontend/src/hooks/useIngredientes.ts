...
const fetchUnidadesMedida = async (): Promise<UnidadMedida[]> => {
  const response = await apiClient.get<any>(`${API_URL}/unidades-medida`);
  return response.data.data || [];
};

const createUnidadMedida = async (data: UnidadMedida): Promise<UnidadMedida> => {
  const response = await apiClient.post<any>(`${API_URL}/unidades-medida`, data);
  return response.data.data || response.data;
};

const deleteUnidadMedida = async (codigo: string): Promise<void> => {
  await apiClient.delete(`${API_URL}/unidades-medida/${codigo}`);
};
