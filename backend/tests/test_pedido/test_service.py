...
from app.venta.schema import PedidoCreate, PedidoUpdate, DetallePedidoCreate, PagoCreate, PagoUpdate
...
def test_create_detalle_pedido(session, catalogo_seed):
    ...
    detalle = pedido_service.create_detalle_pedido(session, detalle_data)
...
def test_update_pago(session, catalogo_seed):
    ...
    updated = pedido_service.update_pago(session, pago, pago_update_dict)
    session.refresh(updated)
