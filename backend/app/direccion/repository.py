...
class DireccionEntregaRepository(BaseRepository[DireccionEntrega]):
    ...
    def get_by_usuario(self, usuario_id: int, limit: int = 100, offset: int = 0) -> tuple[List[DireccionEntrega], int]:
        """Obtiene direcciones de un usuario."""
        statement = select(self.model).where(self.model.usuario_id == usuario_id).offset(offset).limit(limit)
        items = self.session.exec(statement).all()

        count_statement = select(self.model).where(self.model.usuario_id == usuario_id)
    ...
    def get_principal(self, usuario_id: int) -> Optional[DireccionEntrega]:
        ...
        statement = select(self.model).where(
            (self.model.usuario_id == usuario_id) & (self.model.es_principal == True)
        )
